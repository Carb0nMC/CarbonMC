    ...
    public static void loadFromConfig(File file, YamlConfiguration cfg) {
        ...
        for (String key : cfg.getValues(false).keySet()){
            try {
                ...
                CarbonMC.get().getLogger().warning("Error when loading language value \"" + key + "\".");
                e.printStackTrace();
                continue;
            }
        }
        if (!inexistant.isEmpty())
            CarbonMC.get().getLogger().warning("Found " + inexistant.size() + " inexistant string(s) in " + file.getName() + ": " + String.join(" ", inexistant));
    }
